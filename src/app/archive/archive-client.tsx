  ...
  if (tasks.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-8">
        ...
        <h2 className="text-2xl font-semibold text-zinc-200">No completed tasks yet</h2>
        <p className="mt-2 text-zinc-500">Complete your first task to see it here</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto p-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-zinc-400">
          Your completed tasks • {tasks.length} task{tasks.length !== 1 ? "s" : ""} completed
        </p>
      </div>
        ...
        {Object.entries(tasksByDate).map(([date, dateTasks]) => (
          <div key={date}>
            ...
            <div className="space-y-4">
              {dateTasks.map((task) => (
                <div key={task.id} className="opacity-75 transition-opacity hover:opacity-100">
                  <TaskCard
                    task={task}
                    actions={
